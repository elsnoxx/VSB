...
BEGIN
    ...
    SELECT COUNT(*) INTO v_numberOfExcuse FROM OMLUVA WHERE den BETWEEN ADD_MONTHS(:new.den, -1) AND ADD_MONTHS(:new.den, 1) AND ID_POSLANEC = :new.ID_POSLANEC;

    DBMS_OUTPUT.PUT_LINE('Počet omluv:' || v_numberOfExcuse);

    IF v_numberOfExcuse >= 10 THEN
        INSERT INTO TOO_MANY_ABSENCE_LOG (ID_POSLANEC, DATUM_PRIDANI, DEN_OMLUVY) VALUES (:new.ID_POSLANEC, CURRENT_TIMESTAMP, :new.den);
