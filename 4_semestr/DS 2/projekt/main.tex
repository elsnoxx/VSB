...
\begin{enumerate}
    ...
    \item getPriceOfServiceUsage(reservation\_id) – Vrátí seznam služeb využitých v rámci rezervace včetně množství a ceny.
    ...
    \item getReservationDetail(reservation\_id) – Vrátí detailní informace o rezervaci (host, pokoj, platba, služby).
\end{enumerate}
...
Transakce zajistí změnu přiřazeného pokoje pro existující rezervaci. Ověří dostupnost nového pokoje v požadovaném termínu, aktualizuje rezervaci a případně provede další související změny. Vše probíhá atomicky – v případě chyby se provede rollback.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Získej \texttt{check\_in\_date}, \texttt{check\_out\_date}, \texttt{room\_id}, \texttt{accommodation\_price} z tabulky Reservation podle \texttt{reservation\_id}.
    \end{itemize}
    \item \textbf{Ověření dostupnosti nového pokoje:}
    \begin{itemize}
        \item Zjisti, zda je nový pokoj v požadovaném termínu volný:
\begin{verbatim}
SELECT COUNT(*)
FROM Reservation res
...
\end{verbatim}
    ...
    \begin{itemize}
        \item \textbf{Výpočet nové ceny ubytování:}
    \begin{itemize}
        \item Spočítej novou cenu za ubytování podle nového pokoje a termínu (například pomocí funkce \texttt{getPriceOfAccommodation}).
    \end{itemize}
        \item Spočítej novou cenu pomocí funkce \texttt{getPriceOfAccommodation} (použij nový pokoj, check\_in\_date, check\_out\_date).
        \item Aktualizuj záznam v tabulce Reservation – nastav nový \texttt{room\_id} a \texttt{accommodation\_price}:
\begin{verbatim}
UPDATE Reservation
