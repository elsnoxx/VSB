...
\subsection{Detailní popis vybraných funkcí}

\paragraph{Filtrování rezervací podle data příjezdu (getReservationByCheckin)}
Funkce umožňuje uživateli zobrazit pouze rezervace s konkrétním datem příjezdu. Implementováno pomocí SQL dotazu s podmínkou na datum příjezdu.
...
\paragraph{Výpočet ceny ubytování (getPriceOfAccommodation)}
Funkce vypočítá celkovou cenu za ubytování na základě typu pokoje a délky pobytu. Cena je určena jako součin počtu nocí a ceny za noc podle typu pokoje.
...
\paragraph{Přidání služby k rezervaci (addService)}
Funkce umožňuje přidat novou službu k existující rezervaci. Celková cena služby je vypočtena jako součin množství a ceny za jednotku. Vše je zapsáno do tabulky ServiceUsage.
...
\paragraph{Změna pokoje pro rezervaci (changeRoom)}
Funkce umožňuje změnit pokoj u existující rezervace. Nejprve ověří dostupnost nového pokoje v požadovaném termínu. Pokud je pokoj volný, provede změnu v rámci transakce.

\begin{verbatim}
-- Kontrola dostupnosti nového pokoje
SELECT COUNT(*) 
...
\paragraph{Označení platby jako dokončené (payForReservation)}
Funkce označí platbu za rezervaci jako zaplacenou a uloží datum platby. Provádí se aktualizací příslušného záznamu v tabulce Payment.
