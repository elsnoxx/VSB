...
\begin{enumerate}
    \item \textbf{getGuestInformation(reservation\_id)} – Na základě ID rezervace vyhledá příslušného hosta a vrátí jeho osobní údaje (jméno, příjmení, email, telefon, adresa). Interně provádí \texttt{SELECT} s \texttt{JOIN} mezi tabulkami \texttt{Reservation} a \texttt{Guest} přes cizí klíč \texttt{guest\_id}.

    \item \textbf{getReservationList()} – Vrací seznam všech rezervací včetně údajů o hostovi, čísla pokoje, data příjezdu a stavu rezervace. Používá \texttt{SELECT} s vícenásobnými \texttt{JOIN} mezi tabulkami \texttt{Reservation}, \texttt{Guest} a \texttt{Room}.

    \item \textbf{getPriceOfAccommodation(reservation\_id)} – Vrací cenu za ubytování z tabulky \texttt{Reservation}. Jde o jednoduchý \texttt{SELECT} na sloupec \texttt{accommodation\_price} s filtrem přes \texttt{reservation\_id}.

    \item \textbf{getUsedServices(reservation\_id)} – Vrací seznam všech služeb využitých v rámci rezervace včetně názvu, množství a ceny. Využívá \texttt{SELECT} s \texttt{JOIN} mezi \texttt{ServiceUsage} a \texttt{Service}.

    \item \textbf{getPriceOfServices(reservation\_id)} – Spočítá celkovou cenu všech služeb v rámci rezervace pomocí agregační funkce \texttt{SUM(total\_price)} s filtrem dle \texttt{reservation\_id} v tabulce \texttt{ServiceUsage}.

    \item \textbf{getFreeRoomList(check\_in\_date, check\_out\_date)} – Vyhledá všechny pokoje, které nejsou obsazeny v daném termínu. Provádí \texttt{SELECT} z tabulky \texttt{Room} s \texttt{NOT EXISTS} nebo \texttt{LEFT JOIN} a časovým filtrem podle konfliktu s existujícími rezervacemi.

    \item \textbf{getServiceList()} – Vrací seznam všech dostupných služeb. Jednoduchý \texttt{SELECT} z tabulky \texttt{Service}.

    \item \textbf{getReservationState()} – Vrací různé stavy rezervací. Může být realizováno jako \texttt{SELECT DISTINCT status} z tabulky \texttt{Reservation}.
\end{enumerate}

\subsubsection{Funkce typu Invocation (vyvolávací)}

\begin{enumerate}\setcounter{enumi}{8}
    \item \textbf{getReservationByCheckin(check\_in\_date)} – Vrací všechny rezervace, které mají zadané datum příjezdu. Používá \texttt{SELECT} s filtrem na \texttt{check\_in\_date} v tabulce \texttt{Reservation}.

    \item \textbf{changeStatusReservation(reservation\_id, new\_status)} – Aktualizuje stav rezervace na novou hodnotu. Provádí \texttt{UPDATE} v tabulce \texttt{Reservation} na základě \texttt{reservation\_id}.

    \item \textbf{changeRoom(reservation\_id, new\_room\_id)} – Změní přiřazený pokoj v rezervaci. Nejprve ověřuje dostupnost nového pokoje (pomocí \texttt{SELECT}), následně provádí \texttt{UPDATE} tabulky \texttt{Reservation}, případně aktualizuje cenu (např. z \texttt{RoomTypePriceHistory}).

    \item \textbf{payForReservation(payment\_id)} – Označí rezervaci jako zaplacenou a nastaví datum platby. Provádí \texttt{UPDATE} v tabulce \texttt{Payment} na základě \texttt{payment\_id}.

    \item \textbf{addService(reservation\_id, service\_id, quantity, usage\_date)} – Přidá novou službu k rezervaci. Nejprve zjistí aktuální cenu služby (např. z \texttt{ServicePriceHistory}), vypočítá celkovou cenu a vloží nový záznam do tabulky \texttt{ServiceUsage} pomocí \texttt{INSERT}.

    \item \textbf{deleteService(usage\_id)} – Odstraní službu přidělenou k rezervaci. Provádí \texttt{DELETE} z tabulky \texttt{ServiceUsage} podle \texttt{usage\_id}.
\end{enumerate}
...
\begin{verbatim}
SELECT check_in_date, check_out_date, room_id, accommodation_price
FROM Reservation
...
\begin{verbatim}
-- Zjisti počet kolizních rezervací pro daný pokoj a termín
SELECT COUNT(*)
FROM Reservation
...
\end{verbatim}
        \item Pokud je výsledek větší než 0:
        \begin{itemize}
            \item Proveď rollback transakce
            \item Ukonči transakci s chybou "Pokoj není v daném termínu volný"
        \end{itemize}
        \item Jinak pokračuj v transakci
    \end{itemize}
...
WHERE reservation_id = :reservationId;
\end{verbatim}
        \item Zjisti typ pokoje podle nového \texttt{room\_id}:
\begin{verbatim}
SELECT room_type_id
FROM Room
WHERE room_id = :newRoomId;
...
WHERE room_type_id = :roomTypeId
  AND :checkInDate BETWEEN valid_from AND NVL(valid_to, :checkInDate);
\end{verbatim}
        \item Spočítej počet nocí jako rozdíl \texttt{check\_out\_date} a \texttt{check\_in\_date}.
        \item Spočítej novou cenu: \texttt{accommodation\_price = počet\_nocí * cena\_za\_noc}
