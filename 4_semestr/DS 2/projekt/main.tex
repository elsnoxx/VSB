...
\begin{itemize}
    ...
    \item \textbf{Změna pokoje:} Výběrem jiného pokoje v poli „Room no“ se použije funkce \textbf{changeRoom}, která ověří dostupnost a provede změnu.
...
\begin{enumerate}
    ...
    \item \textbf{getPriceOfServices} - Výpočet a zobrazení ceny za jednotlivé služby na základě množství a ceny za jednotku. Funkce načítá data z tabulky Service a počítá celkovou cenu jako součin množství a ceny za jednotku.
    ...
    \item \textbf{getReservationDetail } - Načtení detailních informací o rezervaci na základě ID rezervace. Funkce zobrazuje všechny relevantní informace včetně hosta, pokoje, služeb.
...
\textbf{Algoritmus:}
\begin{enumerate}
    \item Zjisti typ pokoje a termín rezervace podle p\_reservation\_id.
    \item Zjisti cenu za noc z tabulky RoomTypePriceHistory podle data příjezdu.
    \item Spočítej počet nocí (rozdíl check\_out\_date - check\_in\_date).
    \item Vynásob počet nocí cenou za noc.
\end{enumerate}

\textbf{SQL:}
\begin{verbatim}
SELECT 
  (r.check_out_date - r.check_in_date) * rh.price_per_night AS total_price
FROM Reservation r
...
JOIN RoomTypePriceHistory rh ON rh.room_type_id = rt.room_type_id
  AND r.check_in_date BETWEEN rh.valid_from AND NVL(rh.valid_to, r.check_in_date)
...
\textbf{Algoritmus:}
\begin{enumerate}
    \item Zjisti jednotkovou cenu služby z tabulky ServicePriceHistory podle p\_service\_id a data využití služby.
    \item Vypočítej celkovou cenu služby (jednotková cena * množství).
\end{enumerate}

\textbf{SQL:}
\begin{verbatim}
SELECT sph.price * :p_quantity AS total_price
FROM ServicePriceHistory sph
WHERE sph.service_id = :p_service_id
  AND :p_usage_date BETWEEN sph.valid_from AND NVL(sph.valid_to, :p_usage_date);
\end{verbatim}
...
\textbf{Algoritmus:}
\begin{enumerate}
    \item Zjisti cenu služby podle p\_service\_id a p\_usage\_date z tabulky ServicePriceHistory.
    \item Vypočítej celkovou cenu služby (cena * množství).
    \item Vlož záznam do tabulky ServiceUsage.
\end{enumerate}

\textbf{SQL:}
\begin{verbatim}
INSERT INTO ServiceUsage (reservation_id, service_id, quantity, total_price)
SELECT :p_reservation_id, :p_service_id, :p_quantity, (sph.price * :p_quantity)
FROM ServicePriceHistory sph
WHERE sph.service_id = :p_service_id
  AND :p_usage_date BETWEEN sph.valid_from AND NVL(sph.valid_to, :p_usage_date);
\end{verbatim}
...
\textbf{SQL:}
\begin{verbatim}
UPDATE Payment SET is_paid = 1, payment_date = CURRENT_DATE WHERE payment_id = :p_payment_id;
\end{verbatim}
...
\textbf{Algoritmus:}
\begin{enumerate}
    \item Načti základní údaje o rezervaci podle p\_reservation\_id.
    \item Načti údaje o hostovi, pokoji, platbě a využitých službách.
...
\begin{itemize}
    ...
    \item Ošetřuje se souběh více uživatelů (zamykání záznamů).
\end{itemize}
