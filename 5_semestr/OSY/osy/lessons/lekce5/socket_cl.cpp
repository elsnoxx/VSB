...
void* producer_thread(void* arg)
{
    ...
    if (!f) {
        log_msg(LOG_ERROR, "Unable to open %s", g_filename);
        close(sock);
        return NULL;
    }
...
void* consumer_thread(void* arg)
{
    ...
    while (1)
    {
        ...
        int len = readline(sock, buf, sizeof(buf)-1);
        if (len <= 0) break;
        ...
        char receivebuf[MAX_LINE + 1];
        snprintf(receivebuf, sizeof(receivebuf), "%s", buf);
...
int main( int t_narg, char **t_args )
{
    ...
    struct stat myStat;
    if (stat(g_filename, &myStat) == 0){
        strcpy(task, "producer");
    } else{
        strcpy(task, "consumer");
    }
    // task[strcspn(task, "\r\n")] = 0;
