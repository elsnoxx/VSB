...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        {
        //    printf("%s", buf);
            int len = write( l_sock_server, buff, strlen(buff) );
        }
        ...
        char nameFileBin[64];
        char commandBin[96];
        snprintf(nameFile, sizeof(nameFile), "%d.c", (int)pidMain);
        snprintf(nameFileBin, sizeof(nameFileBin), "%d.bin", (int)pidMain);
        snprintf(commandBin, sizeof(commandBin), "./%s", nameFileBin);

        log_msg(LOG_INFO, "Using files: %s %s %s", nameFile, nameFileBin, commandBin);

   
        FILE* pidFile = fopen(nameFile, "w+");

        while (read(l_sock_server, buff, sizeof(buff)) != 0)
        {
            fprintf(pidFile, buff);
            printf("%s", buff);
        }
        ...
        if(pid < 0 ){
            ...
            execlp("gcc", "gcc", nameFile, "-o", nameFileBin);
        } else{
            int l_status;
            waitpid( pid, &l_status, 0 );
            execlp(commandBin, commandBin);
        }
