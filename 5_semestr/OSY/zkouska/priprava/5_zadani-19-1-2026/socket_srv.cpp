...
void childProcess(int sock_client){
    sem_wait(g_sem);
    FILE* imgppm = fopen("img.ppm", "wb");
    char buffer[2048];
    ...
    if(pid < 0 ){
        ...
        dup2(sock_client, STDERR_FILENO);
        execlp("./ppm2png", "./ppm2png", "img.ppm", NULL);
        perror("erro");
        exit(1);
    } else{
        waitpid(pid, NULL, 0);
