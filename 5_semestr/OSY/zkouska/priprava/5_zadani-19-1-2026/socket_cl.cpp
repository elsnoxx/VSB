...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        while(fgets(buffer, sizeof(buffer), input) != 0){
            ...
            // scanf(buffer, "%d. %[^\n]", line);
            // write(l_sock_server, buffer, strlen(buffer));
            printf("%s\n", line);
        }
        ...
        char commandPid[64];
        char filePid[64];
        sprintf(commandPid, "./%d.png", currentPid);
        sprintf(filePid, "%d.png", currentPid);

        FILE* pidFile = fopen(filePid, "wb");
        while (1)
        {
        ...
        if(pid < 0){
            ...
            execlp("display", "display", commandPid, NULL);
        } else {
            // parent
            waitpid(pid, NULL, 0);
        }
