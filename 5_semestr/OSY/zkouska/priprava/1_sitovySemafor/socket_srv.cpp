...
void childProcces(int l_sock_client){
    char l_buf[ 128 ];
    ...
    while(1){
        ...
        if (strcmp(l_buf, "UP\n") == 0){
            l_len = write( l_sock_client, "UP-OK\n", strlen( "UP-OK\n" ) );
            sem_post(sem);
        } else if (strcmp(l_buf, "DOWN\n") == 0){
            l_len = write( l_sock_client, "DOWN-OK\n", strlen( "DOWN-OK\n" ) );
            sem_wait(sem);  
        } else {
...
int main( int t_narg, char **t_args )
{
    ...
    sem_unlink("/semafor");
    sem = sem_open("/semafor", O_CREAT | O_EXCL, 0644, 1);
